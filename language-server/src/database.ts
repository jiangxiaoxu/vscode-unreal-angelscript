...
function sidecarOwnerSymbols(
    ownerName: string,
    symbolName: string,
    kind?: 'method' | 'function' | 'constructor',
) : DBSymbol[]
{
    if (!ownerName || !symbolName)
        return [];
    if (ownerName.startsWith('__'))
    {
        let namespace = LookupNamespace(null, ownerName.substring(2));
        return namespace ? namespace.findSymbols(symbolName) : [];
    }
    // Legacy DebugDatabase stores constructor records in the root namespace,
    // while the access exporter identifies them by their owning type name.
    // Search that canonical root bucket first, then retain the type lookup for
    // engines which keep constructors attached to the type.
    if (kind == 'constructor')
    {
        let rootCandidates = GetRootNamespace().findSymbols(symbolName)
            .filter((symbol) => symbol instanceof DBMethod && symbol.isConstructor);
...
{
    // Preserve const/reference qualifiers: they distinguish real overloads
    // such as `int16&` and `const int16&in`.
    return value.replace(/\s+/g, ' ').trim();
}

function sidecarMethodMatches(
    method: DBMethod,
    args: readonly { type: string }[],
    returnType?: string,
    signature?: string,
    kind?: 'method' | 'function' | 'constructor',
) : boolean
{
    if (kind == 'constructor' && !method.isConstructor)
        return false;
    if (kind == 'method' && (method.containingType == null || method.isConstructor))
        return false;
    if (kind == 'function' && (method.containingType != null || method.isConstructor))
        return false;
    ...
    {
        if (normalizeSidecarArgumentType(methodArgs[index].typename)
            != normalizeSidecarArgumentType(args[index].type))
            return false;
    }
    // Constructor records in the legacy database historically used both the
    // owner type and `void` as their return spelling.  The sidecar's
    // constructor identity is already closed by owner/kind/name/arguments;
    // do not discard its access fact because that legacy-only spelling differs.
    if (kind != 'constructor' && returnType !== undefined
        && normalizeSidecarArgumentType(method.returnType ?? 'void') != normalizeSidecarArgumentType(returnType))
        return false;
    if (signature !== undefined)
    {
        // The legacy record exposes receiver constness separately while the
        // sidecar carries it in the declaration signature.
        let sidecarConst = /\bconst\s*$/.test(signature.trim());
        if (method.isConst != sidecarConst)
            return false;
    }
    return true;
}
...
export function ApplyDebugDatabaseAccess(chunks: readonly DebugDatabaseAccessChunk[]) : void
{
    ...
    {
        for (let property of chunk.properties ?? [])
        {
            for (let symbol of sidecarOwnerSymbols(property.owner, property.name))
            {
                if (symbol instanceof DBProperty && !symbol.declaredModule)
                    symbol.access = property.access;
            }
        }
        for (let method of chunk.methods ?? [])
        {
            let candidates = sidecarOwnerSymbols(method.owner, method.name, method.kind)
                .filter((symbol): symbol is DBMethod => symbol instanceof DBMethod && !symbol.declaredModule)
                .filter((symbol) => sidecarMethodMatches(
                    symbol,
                    method.args,
                    method.return,
                    method.signature,
    ...
    // Legacy Engine responses and older sidecars are valid compatibility paths;
    // expose their native symbols as unknown instead of inferring permissions.
    for (let type of TypesById.values())
    {
        type.forEachSymbol((symbol) => {
            if (symbol.declaredModule)
                return;
            if (symbol instanceof DBProperty && !symbol.access)
                symbol.access = unknownPropertyAccess();
            else if (symbol instanceof DBMethod && !symbol.access)
                symbol.access = unknownFunctionAccess();
        }, false);
    }
    RootNamespace.forEachSymbol((symbol) => {
        if (symbol.declaredModule)
            return;
        if (symbol instanceof DBProperty && !symbol.access)
            symbol.access = unknownPropertyAccess();
